...
function ents.WeaponPhyscannon:AttachParticleEntity(ent,att)
	...
	if(attComponent ~= nil) then
		...
		attInfo.flags = bit.bor(ents.AttachableComponent.FATTACHMENT_MODE_POSITION_ONLY,ents.AttachableComponent.FATTACHMENT_MODE_UPDATE_EACH_FRAME,ents.AttachableComponent.FATTACHMENT_MODE_SNAP_TO_ORIGIN)
...
function ents.WeaponPhyscannon:CreateSprite(att,texture,scale,bloomScale,color,blackToAlpha)
	...
	if(blackToAlpha == true) then ent:SetKeyValue("spawnflags","4096") end
...
function ents.WeaponPhyscannon:CreateLaunchParticles(hitPos)
	...
	if(mdlComponentVm == nil or trComponentVm == nil) then return end
...
function ents.WeaponPhyscannon:InitHoldSprites()
	if(self.m_tHoldSprites ~= nil) then return end
	local vm = ents.get_view_model()
	if(util.is_valid(vm) == false) then return end
	self.m_tHoldSprites = {}
...
function ents.WeaponPhyscannon:DestroyHoldSprites()
	...
	for _,ent in ipairs(self.m_tHoldSprites) do
		if(util.is_valid(ent) == true) then
			...
			if(spriteComponent ~= nil) then spriteComponent:StopAndRemoveEntity()
			else ent:RemoveSafely() end
		end
	end
	self.m_tHoldSprites = nil
end

function ents.WeaponPhyscannon:AreProngsOpen() return (self.m_bProngsOpen == true) and true or false end

function ents.WeaponPhyscannon:OpenProngs()
	if(self:AreProngsOpen() == true) then return end
	self.m_bProngsOpen = true
	local sndEmitterComponent = self:GetEntity():GetComponent(ents.COMPONENT_SOUND_EMITTER)
	if(sndEmitterComponent ~= nil) then sndEmitterComponent:EmitSound("weapon_physcannon.claws_open",sound.TYPE_WEAPON) end
end

function ents.WeaponPhyscannon:CloseProngs()
	if(self:AreProngsOpen() == false) then return end
	self.m_bProngsOpen = false
	local sndEmitterComponent = self:GetEntity():GetComponent(ents.COMPONENT_SOUND_EMITTER)
	if(sndEmitterComponent ~= nil) then sndEmitterComponent:EmitSound("weapon_physcannon.claws_close",sound.TYPE_WEAPON) end
end
