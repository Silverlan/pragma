...
function ents.WeaponPhyscannon:FindPhysProp()
	...
	if(charComponentOwner == nil) then return end
	...
	if(r == false or r.hit == false) then return end
	local physComponent = r.entity:GetComponent(ents.COMPONENT_PHYSICS)
	if(physComponent == nil or physComponent:IsPhysicsProp() == false) then return end
...
function ents.WeaponPhyscannon:OnPrimaryAttack()
	...
	if(CLIENT == true) then
		if(prop == nil) then
			...
			if(wepComponent ~= nil) then wepComponent:PlayViewActivity(Animation.ACT_VM_PRIMARY_ATTACK,ents.AnimatedComponent.FPLAYANIM_RESET) end
		else
			sound.play("weapon_physcannon.launch",sound.TYPE_WEAPON,1.0,1.0)
			if(wepComponent ~= nil) then wepComponent:PlayViewActivity(Animation.ACT_VM_SECONDARY_ATTACK,ents.AnimatedComponent.FPLAYANIM_RESET) end
			local owner = self:GetOwner()
		...
		if(prop ~= nil) then
			...
			if(owner ~= nil and physComponent ~= nil) then
				...
				local dir = (charComponent ~= nil) and charComponent:GetViewForward() or (trComponent ~= nil) and trComponent:GetForward() or nil
				physComponent:ApplyImpulse(dir *800.0 *physComponent:GetMass(),hitPos)
			end
		end
	end
	if(wepComponent ~= nil) then wepComponent:SetNextAttack(time.cur_time() +self.PrimaryAttackDelay) end
end
...
function ents.WeaponPhyscannon:GetHoldPos()
	local owner = self:GetOwner()
	if(owner == nil) then return self:GetPos() end
	local charComponent = owner:GetComponent(ents.COMPONENT_CHARACTER)
	local trComponent = owner:GetComponent(ents.COMPONENT_TRANSFORM)
	if(charComponent == nil or trComponent == nil) then return self:GetPos() end
	return trComponent:GetEyePos() +charComponent:GetViewForward() *100.0 -charComponent:GetViewUp() *10.0 +charComponent:GetViewRight() *10.0
end

function ents.WeaponPhyscannon:DestroyConstraint()
	if(util.is_valid(self.m_constraint) == true) then self.m_constraint:Remove() end
	if(util.is_valid(self.m_rigidBody) == true) then self.m_rigidBody:Remove() end
end

function ents.WeaponPhyscannon:UpdateConstraintPos()
	if(util.is_valid(self.m_rigidBody) == false) then return end
	self.m_rigidBody:SetPos(self:GetHoldPos())
end

function ents.WeaponPhyscannon:IsCarrying() return self.m_bCarrying end

function ents.WeaponPhyscannon:StartCarry(ent)
	if(self:IsCarrying() == true) then return end
	self.m_entTarget = ent
	if(CLIENT == true) then
		local entThis = self:GetEntity()
		local wepComponent = entThis:GetComponent(ents.COMPONENT_WEAPON)
		if(wepComponent ~= nil) then wepComponent:PlayViewActivity(game.Model.Animation.ACT_VM_RELOAD,ents.AnimatedComponent.FPLAYANIM_RESET) end
...
function ents.WeaponPhyscannon:EndHold()
	if(CLIENT == true) then self:DestroyHoldSprites() end
	self:EndCarry()
end

function ents.WeaponPhyscannon:IsPulling() return self.m_bPulling and true or false end

function ents.WeaponPhyscannon:EndCarry()
	self.m_bPulling = false
	if(self:IsCarrying() == false) then return end
	if(CLIENT == true) then
		local ent = self:GetEntity()
		local wepComponent = ent:GetComponent(ents.COMPONENT_WEAPON)
		if(wepComponent ~= nil) then wepComponent:PlayViewActivity(Animation.ACT_VM_IDLE) end
		local sndEmitterComponent = ent:GetComponent(ents.COMPONENT_SOUND_EMITTER)
		if(sndEmitterComponent ~= nil) then sndEmitterComponent:EmitSound("weapon_physcannon.drop",sound.TYPE_WEAPON) end
	end
...
function ents.WeaponPhyscannon:OnTick()
	local wepComponent = self:GetEntity():GetComponent(ents.COMPONENT_WEAPON)
	if(wepComponent == nil or wepComponent:IsDeployed() == false) then return end
	if(CLIENT == true) then self:UpdateProngs() end
	...
	if(CLIENT == true) then
		if(util.is_valid(entPullTarget) == true) then
			...
			self:OpenProngs()
		elseif(self.m_tLastTarget == nil or time.cur_time() -self.m_tLastTarget > 0.8) then self:CloseProngs() end
	end

	local owner = self:GetOwner()
	if((owner:IsPlayer() == false or owner:GetComponent(ents.COMPONENT_PLAYER):GetActionInput(input.ACTION_ATTACK2) == false) and self:IsCarrying() == false) then
		self:EndHold()
		return
	end
		...
		if(util.is_valid(entPullTarget) == true) then
			...
			if(physComponent ~= nil and velComponent ~= nil) then
				...
				if(dist > 0.0 and dist <= self.MaxPullDistance) then
					dir = dir /dist
					velComponent:AddVelocity(dir *self.PullForce *(1.0 -(dist /self.MaxPullDistance)) *time.delta_time()) -- TODO: With relative position!
					if(dist <= 200.0) then self:StartCarry(entPullTarget) end
				end
				self:UpdateConstraintPos()
			end
		end
	else self:EndHold() end
end
ents.COMPONENT_WEAPON_PHYSCANNON = ents.register_component("weapon_physcannon",ents.WeaponPhyscannon,ents.EntityComponent.FREGISTER_BIT_NETWORKED)
