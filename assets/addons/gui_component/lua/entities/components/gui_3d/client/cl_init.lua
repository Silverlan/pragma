...
function ents.GUI3D:GetRefreshRate() return self.m_refreshRate end
function ents.GUI3D:SetMsaaEnabled(enabled) self.m_drawToTexInfo.enableMsaa = enabled end
function ents.GUI3D:IsMsaaEnabled() return self.m_drawToTexInfo.enableMsaa end
...
function ents.GUI3D:ScheduleRender() self.m_renderScheduled = true end
function ents.GUI3D:SetGUIElement(el)
	self.m_pGui = el
end
function ents.GUI3D:GetGUIElement() return self.m_pGui end
...
function ents.GUI3D:CalcCursorPos(origin,dir)
	...
	local p = self.m_pGui
  if(origin == nil or util.is_valid(p) == false) then return end
...
function ents.GUI3D:SetCursor(texture,w,h)
	if(util.is_valid(self.m_pGui) == false) then return end
	w = w or 8
	h = h or 8
...
function ents.GUI3D:SetCursorPos(origin,dir)
	local p = self.m_pGui
	if(util.is_valid(p) == false) then return false end
	...
	if(util.is_valid(self.m_cursor)) then
		...
		if(pos ~= nil) then self.m_cursor:SetPos(pos.x -self.m_cursor:GetWidth() /2,pos.y -self.m_cursor:GetHeight() /2) end
	end
	return pos ~= nil
end
function ents.GUI3D:GetCursorPos()
	if(util.is_valid(self.m_cursor) == false) then return Vector2i() end
	return self.m_cursor:GetPos() +self.m_cursor:GetSize() /2.0
end
function ents.GUI3D:SetUnlit(unlit) self.m_unlit = unlit end
function ents.GUI3D:IsUnlit() return self.m_unlit end
function ents.GUI3D:SetAutoCursorUpdateEnabled(enabled) self.m_autoCursorUpdateEnabled = enabled end
function ents.GUI3D:IsAutoCursorUpdateEnabled() return self.m_autoCursorUpdateEnabled end
function ents.GUI3D:OnTick() self:UpdateCursorPos() end
function ents.GUI3D:UpdateCursorPos()
	if(self.m_interfaceMesh == nil or self.m_autoCursorUpdateEnabled ~= true) then return end
...
function ents.GUI3D:InitializeGUICallbacks()
    ...
    self.m_cbActionInput = pl:AddEventCallback(ents.PlayerComponent.EVENT_HANDLE_ACTION_INPUT,function(action,bPressed,magnitude)
			if(util.is_valid(self.m_pGui) == false) then return util.EVENT_REPLY_UNHANDLED end
			local bt
			if(action == input.ACTION_ATTACK) then bt = input.MOUSE_BUTTON_LEFT
			elseif(action == input.ACTION_ATTACK2) then bt = input.MOUSE_BUTTON_RIGHT end
			if(bt == nil) then return util.EVENT_REPLY_UNHANDLED end
	...
	self.m_cbScrollInput = game.add_callback("OnScrollInput",function(x,y)
		if(util.is_valid(self.m_pGui) == false) then return end
		local pos = self:CalcCursorPos()
		if(pos ~= nil) then self.m_pGui:InjectScrollInput(pos,Vector2(x,y) *10.0) end
		return false
	end)
end
...
function ents.GUI3D:SetClearColor(clearColor) self.m_drawToTexInfo.clearColor = clearColor end
function ents.GUI3D:SetAlwaysRender(alwaysRender) self.m_alwaysRender = alwaysRender end
function ents.GUI3D:GetRenderTarget() return self.m_renderTarget end
function ents.GUI3D:DrawGUIElement()
	if(self.m_alwaysRender) then self.m_renderScheduled = true end
	if(self.m_renderScheduled ~= true) then
		local cursorPos = self.m_cursorPos
		if(cursorPos == self.m_lastRenderCursorPos) then return end
	...
	if(self.m_renderScheduled and self.m_refreshRate ~= -1) then
		...
		if(tSinceLastRender < self.m_tRefreshRate) then return end
		self.m_lastFrameRendered = time.real_time()
	end

	local p = self.m_pGui
	local rt = self.m_renderTarget
	if(util.is_valid(p) == false or rt == nil) then return end
	...
	if(self.m_drawToTexInfo.enableMsaa) then
		...
		drawCmd:RecordImageBarrier(img,prosper.IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,prosper.IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL)
	end
	...
	if(ENABLE_MIPMAPS) then
		drawCmd:RecordImageBarrier(imgDst,prosper.IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,prosper.IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL)
		drawCmd:RecordGenerateMipmaps(imgDst,prosper.IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,prosper.ACCESS_TRANSFER_WRITE_BIT,prosper.PIPELINE_STAGE_TRANSFER_BIT)
	end
end
function ents.GUI3D:InitializeRenderTarget(w,h)
	...
	local dsTex = colTex
	if(msaa) then
		local usageFlags = bit.bor(prosper.IMAGE_USAGE_SAMPLED_BIT,prosper.IMAGE_USAGE_TRANSFER_DST_BIT,prosper.IMAGE_USAGE_TRANSFER_SRC_BIT)
		local imgResolved = gui.create_color_image(w,h,usageFlags,prosper.IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,false --[[msaa]])
		self.m_resolvedTexture = prosper.create_texture(imgResolved,prosper.TextureCreateInfo(),prosper.ImageViewCreateInfo(),prosper.SamplerCreateInfo())
	...
	if(ENABLE_MIPMAPS) then imgCreateInfo.flags = prosper.ImageCreateInfo.FLAG_FULL_MIPMAP_CHAIN_BIT end
	...
	local texDst = prosper.create_texture(imgDst,prosper.TextureCreateInfo(),imgViewCreateInfo,prosper.SamplerCreateInfo())
	...
	local colAttId = rpCreateInfo:AddAttachment(
		...
		prosper.IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,prosper.IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
		prosper.ATTACHMENT_LOAD_OP_DONT_CARE,prosper.ATTACHMENT_STORE_OP_STORE
	)
...
function ents.GUI3D:SetAlphaMode(alphaMode) self.m_alphaMode = alphaMode end
function ents.GUI3D:SetAlphaCutoff(cutoff) self.m_alphaCutoff = cutoff end
function ents.GUI3D:OnEntitySpawn()
	self:InitializeGUICallbacks()
	if(self.m_pGui == nil) then return end
	local mdl = self:InitializeModel(self.m_interfaceMesh)
	local mdlComponent = self:GetEntity():GetComponent(ents.COMPONENT_MODEL)
	if(mdl == nil or mdlComponent == nil) then return end
	
	if(self.m_pGui:IsUpdateScheduled()) then self.m_pGui:Update() end
	self:InitializeGUIDrawCallback()
	self:InitializeRenderTarget(self.m_pGui:GetWidth(),self.m_pGui:GetHeight())
	if(self.m_renderTargetDst == nil) then return end
	local mat = game.create_material(self:IsUnlit() and "unlit" or "pbr")
