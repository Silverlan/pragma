...
function ai.TaskControllerMove:__tostring() return "Move..." end
...
function ai.TaskControllerMove:GetMovementDirection(npc)
	...
	if(trComponent == nil) then return Vector(),0 end
	local controller = npc:GetController()
  local charComponentCtrl = (controller ~= nil) and controller:GetCharacterComponent() or nil
	if(charComponentCtrl == nil) then return trComponent:GetForward(),1.0 end
	local dir = Vector()
	local actionInput = npc:GetControllerActionInput()
	if(bit.band(actionInput,input.ACTION_MOVEFORWARD) ~= 0) then dir = dir +charComponentCtrl:GetViewForward() end
	if(bit.band(actionInput,input.ACTION_MOVEBACKWARD) ~= 0) then dir = dir -charComponentCtrl:GetViewForward() end
	if(bit.band(actionInput,input.ACTION_MOVERIGHT) ~= 0) then dir = dir +charComponentCtrl:GetViewRight() end
	if(bit.band(actionInput,input.ACTION_MOVELEFT) ~= 0) then dir = dir -charComponentCtrl:GetViewRight() end
	local l = dir:Length()
	if(l > 0.0) then dir = dir /l end
	return dir,l
end
...
function ai.TaskControllerMove:Start(schedule,npc)
  ...
  local trComponent = npc:GetEntity():GetTransformComponent()
	...
	if(bit.band(npc:GetControllerActionInput(),input.ACTION_SPRINT) ~= 0) then moveInfo.activity = Animation.ACT_RUN end
