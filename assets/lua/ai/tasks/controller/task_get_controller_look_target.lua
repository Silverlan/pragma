...
function ai.TaskGetControllerLookTarget:__tostring()
	return "Get Controller Look Target"
end
...
function ai.TaskGetControllerLookTarget:Start(schedule, npc)
	...
	if charComponentCtrl == nil or trComponent == nil then
		return ai.BehaviorTask.RESULT_FAILED
	end
	self:SetParameterVector(
		self.RETURN_LOOK_TARGET,
		trComponent:GetEyePos() + charComponentCtrl:GetViewForward() * 8192
	)
	return ai.BehaviorTask.RESULT_SUCCEEDED
end
ai.TASK_GET_CONTROLLER_LOOK_TARGET = ai.TASK_GET_CONTROLLER_LOOK_TARGET
	or ai.register_task(ai.TaskGetControllerLookTarget)
