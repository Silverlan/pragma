...
skin["witooltip"] = {
	Initialize = function(GUI,pElement)
		...
		local cbSize = pText:AddCallback("SetSize",function(pText)
			...
			if(pElement:GetRight() >= szWindow.x) then pElement:SetX(szWindow.x -pElement:GetWidth()) end
			if(pElement:GetBottom() >= szWindow.y) then pElement:SetY(szWindow.y -pElement:GetHeight()) end
		end)
