...
function Component:EndRecording()
	if(self.m_recording ~= true) then return end
...
function Component:GetAnimation() return self.m_animation end
function Component:IsRecording() return self.m_recording end

function Component:RecordEntity(channelName,time,entityInfo)
	if(self.m_animations[channelName] == nil) then return end
...
function Component:InitializeChannels(entityInfo)
	if(entityInfo.entity:IsValid() == false) then return end
	...
	for componentName,props in pairs(entityInfo.properties) do
		...
		if(c == nil) then self:Log("Unable to add animation channels for component '" .. componentName .. "' of entity " .. tostring(entityInfo.entity) .. ": Component does not exist!")
		else
			for _,propName in ipairs(props) do
				local idx = c:GetMemberIndex(propName)
				if(idx == nil) then self:Log("Unable to add animation channel for property '" .. propName .. "' of component '" .. componentName .. "' of entity " .. tostring(entityInfo.entity) .. ": Member does not exist!")
				else
...
ents.COMPONENT_GAME_ANIMATION_RECORDER = ents.register_component("game_animation_recorder",Component,ents.EntityComponent.FREGISTER_BIT_NETWORKED)
