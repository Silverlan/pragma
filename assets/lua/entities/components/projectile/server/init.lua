...
function ents.ProjectileComponent:InitializeHealth(component)
	...
	self:FlagCallbackForRemoval(
		healthProp:AddCallback(function(oldHealth, newHealth)
			if newHealth > 0 then
				return
			end
			self:OnHit()
		end),
		ents.EntityComponent.CALLBACK_TYPE_ENTITY
	)
end

function ents.ProjectileComponent:OnHit()
	if self.m_bHit == true then
		return
	end
	self.m_bHit = true
...
function ents.ProjectileComponent:OnStartTouch(entTouch, physTouch)
	...
	if entTouch ~= nil then
		self:Hit(entTouch, physTouch)
	end
	self:OnHit()
end
...
function ents.ProjectileComponent:Hit(ent, phys)
	...
	if self.m_attacker ~= nil then
		dmg:SetAttacker(self.m_attacker)
	end
	...
	ent:TakeDamage(dmg)
	if self.m_bHit ~= true then
		self:BroadcastEvent(self.EVENT_ON_PROJECTILE_IMPACT)
	end
end
...
function ents.ProjectileComponent:SetAttacker(attacker)
	...
	if util.is_valid_entity(attacker) == false or physComponent == nil then
		return
	end
	local physComponentAttacker = attacker:GetComponent(ents.COMPONENT_PHYSICS)
	if physComponentAttacker == nil then
		return
	end
	physComponent:DisableCollisions(physComponentAttacker)
end
function ents.ProjectileComponent:GetAttacker()
...
function ents.ProjectileComponent:Think(tDelta)
	...
	if t >= self.m_tRemove then
		self:Remove()
	end
end

function ents.ProjectileComponent:CanTrigger(phys)
	return true
end
