...
function Component:GetDrawSceneInfo() return self.m_drawSceneInfo end

function Component:SetCamera(cam) self.m_targetCamera = cam end
...
function Component:ClearScene()
	...
	if(self.m_customCamera == true and util.is_valid(self.m_camera)) then util.remove(self.m_camera:GetEntity()) end
end
...
Component.EVENT_ON_RENDER_SCENE_INITIALIZED = ents.register_component_event(ents.COMPONENT_RENDER_TARGET,"on_render_scene_initialized")
