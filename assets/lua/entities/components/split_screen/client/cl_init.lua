...
function ents.SplitScreen:OnRemove()
	...
	for _,sceneData in ipairs(self.m_scenes) do
		if(util.is_valid(sceneData.scene)) then util.remove(sceneData.scene) end
		if(sceneData.ownCamera and util.is_valid(sceneData.camera)) then util.remove(sceneData.camera:GetEntity()) end
		if(sceneData.renderer:IsValid()) then util.remove(sceneData.renderer:GetEntity()) end
	end
...
function ents.SplitScreen:OnEntitySpawn()
	...
	local elTexLeft = gui.create("WITexturedRect",self.m_uiEl,0,0,self.m_uiEl:GetWidth() /2,self.m_uiEl:GetHeight(),0,0,1,1)
	elTexLeft:SetTexture(self.m_scenes[ents.SplitScreen.SCREEN_LEFT].renderer:GetPresentationTexture())

	local elTexRight = gui.create("WITexturedRect",self.m_uiEl,elTexLeft:GetRight(),0,self.m_uiEl:GetWidth() /2,self.m_uiEl:GetHeight(),0,0,1,1)
