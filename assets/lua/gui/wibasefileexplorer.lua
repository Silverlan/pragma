...
function gui.BaseFileExplorer:SetFilter(filter) self.m_filter = filter end
...
function gui.BaseFileExplorer:GetRootPath() return self.m_rootPath end
function gui.BaseFileExplorer:IsAtRoot() return self.m_path == "/" end
function gui.BaseFileExplorer:GetPath()
	local path = self.m_path
	if(#path > 0 and path:sub(1,1) == "/") then path = path:sub(2) end
	return path
end
...
function gui.BaseFileExplorer:IsValidExtension(ext)
	if(self.m_tExtensions == nil) then return true end
...
function gui.BaseFileExplorer:FindFiles()
	...
	if(self.m_isDirExplorer ~= true) then table.sort(tFiles) end
