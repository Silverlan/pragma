...
function gui.BaseHUD:OnInitialize()
	...
	if(healthComponent ~= nil) then
		self.m_cbHealthChanged = healthComponent:AddEventCallback(ents.HealthComponent.EVENT_ON_HEALTH_CHANGED,function(oldHealth,health)
	...
	if(charComponent ~= nil) then
		self.m_cbWeaponChanged = charComponent:AddEventCallback(ents.CharacterComponent.EVENT_ON_SET_ACTIVE_WEAPON,function(oldWep,weapon)
...
function gui.BaseHUD:OnRemove()
	...
	for _,cb in ipairs({self.m_cbHealthChanged,self.m_cbMaxHealthChanged,self.m_cbWeaponChanged}) do
		if(util.is_valid(cb) == true) then cb:Remove() end
	end
...
function gui.BaseHUD:OnHealthChanged(oldHealth,newHealth)
end
function gui.BaseHUD:GetWeapon() return self.m_activeWeapon end
function gui.BaseHUD:OnPrimaryClipSizeChanged(oldSize,newSize)
end
function gui.BaseHUD:OnSecondaryClipSizeChanged(oldSize,newSize)
end
function gui.BaseHUD:OnWeaponChanged(weapon)
	self:ClearWeaponHooks()
	self.m_activeWeapon = weapon
	if(weapon == nil) then return end
	self.m_cbPrimaryClipChanged = weapon:AddEventCallback(ents.WeaponComponent.EVENT_ON_PRIMARY_CLIP_SIZE_CHANGED,function(oldSize,size)
		self:OnPrimaryClipSizeChanged(oldSize,size)
	end)
	self.m_cbSecondaryClipChanged = weapon:AddEventCallback(ents.WeaponComponent.EVENT_ON_SECONDARY_CLIP_SIZE_CHANGED,function(oldSize,size)
