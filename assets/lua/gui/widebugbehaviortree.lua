...
function gui.WIDebugBehaviorTree:FocusStep()
	...
	self:SetPos(pos)
	if sc >= 1.0 then
		self.m_focusInfo = nil
	end
end
function gui.WIDebugBehaviorTree:GetRootNodes()
	return self.m_rootNodes
end
function gui.WIDebugBehaviorTree:GetNodes()
	return self.m_nodes
end
function gui.WIDebugBehaviorTree:GetNode(i)
...
function gui.WIDebugBehaviorTree:AddNode(node, parent)
	...
	if parent ~= nil then
		...
		table.insert(self.m_rootNodes, p)
	end
	return p
end
function gui.WIDebugBehaviorTree:ClearNodes()
	...
	self.m_nodes = {}
	self.m_rootNodes = {}
	if util.is_valid(self.m_pHighlight) then
		self.m_pHighlight:SetVisible(false)
	end
end
function gui.WIDebugBehaviorTree:BuildTree(tree)
	...
	if update == false then
		...
		local function update_node_positions(i, yStart)
			...
			for _, n in ipairs(nodeLevels[i]) do
				...
				if p ~= nil and np ~= p then
					yStart = yStart + NODE_HORIZONTAL_SPACING_GROUP
				end
			...
			if #nodeLevels > i then
				...
				for childIdx, n in ipairs(nodeLevels[i + 1]) do
					...
					if childIdx > 1 then
						hChildren = hChildren + NODE_HORIZONTAL_SPACING
					end

					local np = n:GetParentNode()
					if p ~= nil and np ~= p then
						hChildren = hChildren + NODE_HORIZONTAL_SPACING_GROUP
					end
					p = np
				end
	...
	elseif util.is_valid(self.m_pHighlight) then
		self.m_pHighlight:SetVisible(false)
	end
end
function gui.WIDebugBehaviorTree:FocusOnNode(node, xPos, yPos, duration)
	if node == self.m_focusedNode then
		return
	end
	yPos = yPos or 0.5
...
function gui.WIDebugBehaviorTreeNode:GetName()
	return self.m_name
end
function gui.WIDebugBehaviorTreeNode:IsActive()
	return self.m_bActive
end
function gui.WIDebugBehaviorTreeNode:GetResult()
	return self.m_result
end
function gui.WIDebugBehaviorTreeNode:SetResult(result)
	local col = NODE_BACKGROUND_COLOR_INITIAL
	if result == RESULT_SUCCEEDED then
		col = NODE_BACKGROUND_COLOR_SUCCESS
	elseif result == RESULT_FAILED then
		col = NODE_BACKGROUND_COLOR_FAILURE
	elseif result == RESULT_PENDING then
		col = NODE_BACKGROUND_COLOR_PENDING
	end
...
function gui.WIDebugBehaviorTreeNode:SetNode(node)
	...
	self.m_pText:SetPos(
		self:GetWidth() * 0.5 - self.m_pText:GetWidth() * 0.5,
		self:GetHeight() * 0.5 - self.m_pText:GetHeight() * 0.5
	)
...
function gui.WIDebugBehaviorTreeNode:GetParentNode()
	return self.m_parentNode
end
function gui.WIDebugBehaviorTreeNode:GetParentConnector()
	return self.m_parentConnector
end
...
function gui.WIDebugBehaviorTreeNode:SetChildNodes(childNodes)
	self.m_childNodes = childNodes
end
function gui.WIDebugBehaviorTreeNode:SetChildConnectors(connectors)
	self.m_childConnectors = connectors
end
function gui.WIDebugBehaviorTreeNode:GetChildNodes()
