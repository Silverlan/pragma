...
function gui.DebugRenderStats:OnInitialize()
	...
	self.m_cbActionInput = pl:AddEventCallback(ents.PlayerComponent.EVENT_HANDLE_ACTION_INPUT,function(action,bPressed,magnitude)
	...
	self:AddSlider({self.m_tree:GetRoot(),self.m_data},"Scenes:",function(stats) return self.m_numScenes end)
	-- self:AddSlider({self.m_tree:GetRoot(),self.m_data},"FPS:",function(stats) return end)
	self:AddSlider({self.m_tree:GetRoot(),self.m_data},"[GPU] Frame Execution Time:",function(stats) return self.m_engineGpuRenderTimes["frame"] or 0 end,true)
	self:AddSlider({self.m_tree:GetRoot(),self.m_data},"[GPU] GUI Render Time:",function(stats) return self.m_engineGpuRenderTimes["gui"] or 0 end,true)
	self:AddSlider({self.m_tree:GetRoot(),self.m_data},"[GPU] Scene Render Time:",function(stats) return self.m_engineGpuRenderTimes["scene"] or 0 end,true)
	self:AddSlider({self.m_tree:GetRoot(),self.m_data},"[GPU] Present Time:",function(stats) return self.m_engineGpuRenderTimes["present"] or 0 end,true)
	-- self:AddSlider({self.m_tree:GetRoot(),self.m_data},"Total CPU Execution Time:",function(stats) return end,true)
end
function gui.DebugRenderStats:SetLocked(locked) self.m_locked = locked end
function gui.DebugRenderStats:IsLocked() return self.m_locked or false end
function gui.DebugRenderStats:InitializeUiElements()
	...
	self.m_data = gui.create("WIVBox",dataScrollContainer,0,0,dataScrollContainer:GetWidth(),dataScrollContainer:GetHeight())
...
function gui.DebugRenderStats:OnThink()
	...
	if(self.m_tNextUpdate > t or self:IsLocked()) then return end
	...
	for _,sceneStats in ipairs(self.m_curStats) do
		...
		for i=0,game.RenderStats.RENDER_PASS_COUNT -1 do
			sceneStats.cpuExecTime = sceneStats.cpuExecTime +self:GetRenderPassTime(sceneName,i,game.RenderPassStats.TIMER_CPU_EXECUTION)
			sceneStats.gpuExecTime = sceneStats.gpuExecTime +self:GetRenderPassTime(sceneName,i,game.RenderPassStats.TIMER_GPU_EXECUTION)
			sceneStats.drawCalls = sceneStats.drawCalls +self:GetRenderCounter(sceneName,i,game.RenderPassStats.COUNTER_DRAW_CALLS)
			sceneStats.numTris = sceneStats.numTris +self:GetRenderCounter(sceneName,i,game.RenderPassStats.COUNTER_DRAWN_TRIANGLES)
		end
	...
	for _,sliderData in ipairs(self.m_sliders) do
		if(sliderData.element:IsValid()) then
			...
			if(val) then self:UpdateSlider(sliderData.element,val) end
		end
	end
end
function gui.DebugRenderStats:GetTime(sceneName,timer)
	if(self.m_sceneNameToStats[sceneName] == nil) then return end
...
function gui.DebugRenderStats:AddScene(statsData)
	...
	if(self.m_scenes[sceneName] == nil) then
		...
		self:AddSlider(sceneCat,"[CPU] Execution Time:",function(stats) return self.m_sceneNameToStats[sceneName] and self.m_sceneNameToStats[sceneName].cpuExecTime or 0.0 end,true)
		self:AddSlider(sceneCat,"[GPU] Execution Time:",function(stats) return self.m_sceneNameToStats[sceneName] and self.m_sceneNameToStats[sceneName].gpuExecTime or 0.0 end,true)
		self:AddSlider(sceneCat,"[GPU] Light Culling GPU Execution Time:",function(stats) return self:GetTime(sceneName,game.RenderStats.TIMER_LIGHT_CULLING_GPU) end,true)
		...
		self:AddSlider(pp,"[GPU] Fog:",function(stats) return self:GetTime(sceneName,game.RenderStats.TIMER_POST_PROCESSING_GPU_FOG) end,true)
		self:AddSlider(pp,"[GPU] Bloom:",function(stats) return self:GetTime(sceneName,game.RenderStats.TIMER_POST_PROCESSING_GPU_BLOOM) end,true)
		self:AddSlider(pp,"[GPU] Tone Mapping:",function(stats) return self:GetTime(sceneName,game.RenderStats.TIMER_POST_PROCESSING_GPU_TONE_MAPPING) end,true)
		self:AddSlider(pp,"[GPU] FXAA:",function(stats) return self:GetTime(sceneName,game.RenderStats.TIMER_POST_PROCESSING_GPU_FXAA) end,true)
		self:AddSlider(pp,"[GPU] SSAO:",function(stats) return self:GetTime(sceneName,game.RenderStats.TIMER_POST_PROCESSING_GPU_SSAO) end,true)

		self:AddSlider(sceneCat,"Draw Calls:",function(stats) return self.m_sceneNameToStats[sceneName] and self.m_sceneNameToStats[sceneName].drawCalls or 0 end)
		self:AddSlider(sceneCat,"Triangles:",function(stats) return self.m_sceneNameToStats[sceneName] and self.m_sceneNameToStats[sceneName].numTris or 0 end)
		local passes = {
		...
		for passId,name in pairs(passes) do
			...
			self:AddSlider(cat,"CPU Wait Time (+Execution):",function(stats) return self:GetRenderPassTime(sceneName,passId,game.RenderPassStats.TIMER_CPU_EXECUTION) end,true)
			self:AddSlider(cat,"GPU Execution Time:",function(stats) return self:GetRenderPassTime(sceneName,passId,game.RenderPassStats.TIMER_GPU_EXECUTION) end,true)

			local catCounters = self:AddCategory(cat,"Counters")
			self:AddSlider(catCounters,"Draw Calls:",function(stats) return self:GetRenderCounter(sceneName,passId,game.RenderPassStats.COUNTER_DRAW_CALLS) end)
			self:AddSlider(catCounters,"Triangles:",function(stats) return self:GetRenderCounter(sceneName,passId,game.RenderPassStats.COUNTER_DRAWN_TRIANGLES) end)
			self:AddSlider(catCounters,"Shader State Changes:",function(stats) return self:GetRenderCounter(sceneName,passId,game.RenderPassStats.COUNTER_SHADER_STATE_CHANGES) end)
			self:AddSlider(catCounters,"Material State Changes:",function(stats) return self:GetRenderCounter(sceneName,passId,game.RenderPassStats.COUNTER_MATERIAL_STATE_CHANGES) end)
			self:AddSlider(catCounters,"Entity State Changes:",function(stats) return self:GetRenderCounter(sceneName,passId,game.RenderPassStats.COUNTER_ENTITY_STATE_CHANGES) end)
			self:AddSlider(catCounters,"Meshes:",function(stats) return self:GetRenderCounter(sceneName,passId,game.RenderPassStats.COUNTER_DRAWN_MESHES) end)
			self:AddSlider(catCounters,"Instance Sets:",function(stats) return self:GetRenderCounter(sceneName,passId,game.RenderPassStats.COUNTER_INSTANCE_SETS) end)
		end
	end
...
function gui.DebugRenderStats:AddSlider(cat,label,f,timer)
	...
	if(timer) then elSlider:SetPostFix(" ms") end
...
function gui.DebugRenderStats:UpdateSlider(el,value)
	...
	if(value > max) then el:SetRange(min,value,stepSize) end
	el:SetValue(value)
end
