...
function gui.WIFileDialog:MakeDirectoryExplorer() if(util.is_valid(self.m_pFileList) == true) then self.m_pFileList:MakeDirectoryExplorer() end end
...
gui.create_file_open_dialog = function(fcOnFileSelected)
	local dialog,frame,fileDialog = gui.create_dialog(function()
		...
		if(fcOnFileSelected ~= nil) then el:AddCallback("OnFileSelected",fcOnFileSelected) end
		return el
	end)
