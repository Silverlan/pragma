...
function gui.WIMenuBar:UpdateWindowColors()
	if(self.m_eventData == nil) then return end
...
get_event = function()
	...
	for name,evData in pairs(events) do
		if(evData.month == currentMonth and evData.day == currentDay) then return evData end
		if(evData.day == nil and evData.month == currentMonth) then fallback = evData end
	end
	return fallback
end
