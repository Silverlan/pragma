...
local function initialize_camera_buffer(self)
	...
	if(shaderGeometry == nil) then return false end
	self.m_shaderGeometry = shaderGeometry
	self.m_shaderShaded = shader.get("lve_textured")
	local descBuffer = shaderGeometry:GenerateSwapDescriptorBuffer(2,prosper.BUFFER_USAGE_UNIFORM_BUFFER_BIT,util.SIZEOF_MAT4 *3)
...
local function get_nearz() return -8000.0 end
...
function gui.WIOrthoView:UpdateCameraBuffer()
	...
	local p = matrix.create_orthogonal_matrix(-startPos[axes[1]],-endPos[axes[1]],-endPos[axes[2]],-startPos[axes[2]],get_farz(),get_nearz())
	local vp = p *v
...
local function render_objects(self)
	if(self.m_tRender == nil) then return end
	local drawCmd = game.get_draw_command_buffer()

	local shaders = {}
	table.insert(shaders,self.m_shaderGeometry)
	if(self.m_shaderShaded ~= nil) then table.insert(shaders,self.m_shaderShaded) end
	...
	for i=#shaders,1,-1 do
		...
		if(shader:BeginDraw() == true) then
			if(i == 1) then drawCmd:SetLineWidth(1.0) end
			drawCmd:SetViewport(w,h)
			drawCmd:SetScissor(w,h)
			if(i == 1) then shader:BindColor(Color.White) end
...
function gui.WIOrthoView:GetTextureElement() return self.m_pBg end
function gui.WIOrthoView:OnInitialize()
	gui.Base.OnInitialize(self)
	if(initialize_camera_buffer(self) == false) then return end

	local pBg = gui.create("WITexturedRect",self)
	pBg:SetAutoAlignToParent(true)
	self.m_pBg = pBg

	local err,rt = prosper.create_render_target(RENDER_TARGET_SIZE.x,RENDER_TARGET_SIZE.y,prosper.FORMAT_R8G8B8A8_UNORM,bit.bor(prosper.IMAGE_USAGE_COLOR_ATTACHMENT_BIT,prosper.IMAGE_USAGE_SAMPLED_BIT),prosper.FORMAT_D32_SFLOAT,true,false,true)
	...
	self.m_cbRender = game.add_callback("PreRender",function()
		if(self:IsValid() == false or self.m_bRenderScheduled ~= true) then return end
