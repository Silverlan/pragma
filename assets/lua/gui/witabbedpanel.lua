...
function gui.WITabbedPanelTab:Update()
	if(util.is_valid(self.m_pTab) == false) then return end
...
function gui.WITabbedPanelTab:SetSelected(b)
	if(b == self.m_bSelected) then return end
	self:CallCallbacks("OnSelected",b)
	self.m_bSelected = b
	self:Update()
end
function gui.WITabbedPanelTab:IsSelected() return self.m_bSelected end
function gui.WITabbedPanelTab:GetTabElement() return self.m_pTab end
...
function gui.WITabbedPanel:FindTab(name) return self.m_tabIdentifierToTab[name] end
function gui.WITabbedPanel:AddTab(name)
	local t = gui.create("WITabbedPanelTab",self)
	t:SetTitle(name)
	t:AddCallback("OnSelected",function(t,b)
		if(b == false or self:IsValid() == false) then return end
		local idx
...
function gui.WITabbedPanel:Update()
	...
	for _,tabData in ipairs(self.m_tTabs) do
		...
		if(elTab:IsValid() == true) then
			if(elTab:IsSelected() == true) then bGotSelected = true end
	...
	if(bGotSelected == false and #self.m_tTabs > 0) then
		...
		if(el:IsValid() == true) then el:SetSelected(true) end
	end
end
