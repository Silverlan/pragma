...
function util.ImageProcessor.Stage:SetEnabled(enabled) self.m_enabled = enabled end
function util.ImageProcessor.Stage:IsEnabled() return self.m_enabled end
...
function util.ImageProcessor:GetInputTexture() return (self.m_inputTexture ~= nil) and self.m_inputTexture.texture or nil end
function util.ImageProcessor:GetTexture(i) return (self.m_textures[i] ~= nil) and self.m_textures[i].texture or nil end

function util.ImageProcessor:GetWidth() return self.m_width end
function util.ImageProcessor:GetHeight() return self.m_height end

function util.ImageProcessor:AddStage(name,stage)
	if(type(stage) == "function") then stage = util.ImageProcessor.Stage(stage) end
	table.insert(self.m_stages,stage)
	self.m_nameToStage[name] = stage
end

function util.ImageProcessor:SetStageEnabled(name,enabled)
	if(self.m_nameToStage[name] == nil) then return end
...
function util.ImageProcessor:AcquireTexture(drawCmd)
	...
	if(prevTex ~= nil and #self.m_textures == 1) then return end
	local tex = self.m_textures[curTex]
	if(tex == nil) then return end
	self.m_prevTexture = curTex
	self.m_curTexture = self.m_curTexture +1
	if(self.m_curTexture == #self.m_textures +1) then self.m_curTexture = 1 end
	...
	if(dsSrc == nil) then dsSrc = self.m_inputTexture.descriptorSet end
...
function util.ImageProcessor:AddStagingTexture(createInfo)
	createInfo.usageFlags = bit.bor(createInfo.usageFlags,prosper.IMAGE_USAGE_COLOR_ATTACHMENT_BIT,prosper.IMAGE_USAGE_SAMPLED_BIT)
	...
	rpCreateInfo:AddAttachment(
		createInfo.format,prosper.IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,prosper.IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
