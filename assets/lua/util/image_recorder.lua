...
function util.ImageRecorder:IsRecording() return self.m_recording end
function util.ImageRecorder:GetFrameDeltaTime() return (1.0 /self.m_frameRate) end
function util.ImageRecorder:SetFrameRate(frameRate) self.m_frameRate = frameRate end
function util.ImageRecorder:SetImage(img) self.m_image = img end
function util.ImageRecorder:RenderNextFrame()
	...
	self.m_cbTick = game.add_callback("Tick",function()
		...
		self.m_cbRender = game.add_callback("PostRender",function()
			...
			if(r ~= true) then self:Log("Unable to save frame '" .. fileName .. "'!",true) end
