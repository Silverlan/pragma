...
local function update_lightmap_data(tEnts)
	...
	for _,ent in ipairs(ents.get_all()) do ent:RemoveComponent(ents.COMPONENT_LIGHT_MAP_RECEIVER) end
...
util.load_baked_lightmap_uvs = function(lightmapCachePath,tEnts)
	...
	log.msg("Failed to load lightmap UV cache from '" .. lightmapCachePath .. "'!",pfm.LOG_CATEGORY_LOG_CATEGORY_LIGHTMAP,pfm.LOG_SEVERITY_WARNING)
	update_lightmap_data(tEnts)
	return true
end
...
util.bake_lightmaps = function(preview,lightIntensityFactor)
	...
	if(result) then log.msg("Successfully created render job for scene lightmap atlas!",pfm.LOG_CATEGORY_LOG_CATEGORY_LIGHTMAP)
	else log.msg("Failed to create render job for scene lightmap atlas!",pfm.LOG_CATEGORY_LOG_CATEGORY_LIGHTMAP,pfm.LOG_SEVERITY_WARNING) end
	...
	if(tEnts == nil) then
		...
		for _,ent in ipairs(ents.get_all()) do
			...
			if(includeEntityForLightmaps) then
				log.msg("Including entity '" .. tostring(ent) .. "' for lightmap atlas!",pfm.LOG_CATEGORY_LOG_CATEGORY_LIGHTMAP)
