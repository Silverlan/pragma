function(pr_add_header_list TARGET_NAME HEADER_LIST)
    ...
    cmake_parse_arguments(PARSE_ARGV 0 PA "${options}" "${oneValueArgs}"
                          "${multiValueArgs}")
    ...
    foreach(source IN LISTS HEADER_LIST)
        ...
        string(REPLACE "${CMAKE_CURRENT_LIST_DIR}" "" source_path_relative
                       "${source_path}")
...
function(pr_add_headers TARGET_NAME HEADER_LOCATION)
    ...
    message(
        "[PR] Adding include directory \"${HEADER_LOCATION}\" to target ${TARGET_NAME} with visibility ${VISIBILITY}"
    )
    ...
    message(
        "[PR] Adding headers in location directory \"${CMAKE_SOURCE_DIR}/${HEADER_LOCATION}\"..."
    )
    file(GLOB_RECURSE HEADER_LIST "${HEADER_LOCATION}/*.h"
         "${HEADER_LOCATION}/*.hpp")
    pr_add_header_list(${TARGET_NAME} "${HEADER_LIST}" "${BASE_DIRS}" BASE_DIRS
                       "${PA_BASE_DIRS}")
endfunction()
...
function(pr_add_modules TARGET_NAME MODULE_LOCATION)
    message(
        "[PR] Adding modules in location directory \"${CMAKE_SOURCE_DIR}/${MODULE_LOCATION}\"..."
    )
...
function(pr_add_sources TARGET_NAME SOURCE_LOCATION)
    set(VISIBILITY PRIVATE)
    message(
        "[PR] Adding include directory \"${SOURCE_LOCATION}\" to target ${TARGET_NAME} with visibility ${VISIBILITY}"
    )
    target_include_directories(${TARGET_NAME} ${VISIBILITY}
                               "${SOURCE_LOCATION}")

    message(
        "[PR] Adding sources in location directory \"${CMAKE_SOURCE_DIR}/${SOURCE_LOCATION}\"..."
    )
    file(GLOB_RECURSE SOURCE_LIST "${SOURCE_LOCATION}/*.c"
         "${SOURCE_LOCATION}/*.cpp")
...
function(pr_add_compile_definitions TARGET_NAME)
    ...
    message(
        "[PR] Adding compile definitions ${PA_UNPARSED_ARGUMENTS} to target ${TARGET_NAME} with visibility ${VISIBILITY}"
    )
