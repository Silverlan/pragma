...
void WIConsole::Initialize()
{
	...
	pEntry->SetAutocompleteHandler([this,pEntry,hThis](const std::string &cmd,std::vector<std::string> &args) {
		...
		const auto fProcessConVars = [&cmd,&bestCandidates](const std::map<std::string,std::shared_ptr<ConConf>> &conVars) {
			for(auto &pair : conVars)
			{
				...
				auto it = std::find_if(bestCandidates.begin(),bestCandidates.end(),[percentage](const std::pair<std::string_view,float> &pair) {
					return percentage < pair.second;
				});
	...
	pLog->GetTextElement()->SetTagArgument("luafile",0u,FunctionCallback<util::EventReply,std::reference_wrapper<const std::vector<std::string>>>::CreateWithOptionalReturn(
		[](util::EventReply *result,std::reference_wrapper<const std::vector<std::string>> args) -> CallbackReturnType {
	...
	if(pMainMenu)
	{
		m_cbMainMenuVisibility = pMainMenu->GetVisibilityProperty()->AddCallback([this](std::reference_wrapper<const bool> oldValue,std::reference_wrapper<const bool> visible) {
			UpdateConsoleMode();
		});
	}
}
...
WICommandLineEntry *WIConsole::GetCommandLineEntryElement()
{
	return static_cast<WICommandLineEntry*>(m_hCommandEntry.get());
}
WITextEntry *WIConsole::GetTextLogElement()
{
	return static_cast<WITextEntry*>(m_hLog.get());
}
...
void WIConsole::InitializeSnapAreas()
{
	...
	CreateSnapTarget(
		0,0,REF_WIDTH,REF_HEIGHT,
		0,0,sizeTrigger,sizeTrigger
	);
	auto *pTopSnapTarget = CreateSnapTarget(
		0,0,REF_WIDTH,size,
		0,0,REF_WIDTH,sizeTrigger
	);
	CreateSnapTarget(
		0,REF_HEIGHT -size,REF_WIDTH,size,
		0,size -sizeTrigger,REF_WIDTH,sizeTrigger
	);
