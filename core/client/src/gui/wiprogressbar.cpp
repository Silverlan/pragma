...
WIProgressBar::WIProgressBar()
	: WIBase(),m_progress(std::make_shared<util::FloatProperty>(0.f)),
	m_min(0.f),m_max(100.f),m_stepSize(1.f),
	m_numDecimals(0)
{
...
void WIProgressBar::SetOptions(const std::vector<std::string> &options) {m_options = options;UpdateOptions();}
void WIProgressBar::AddOption(const std::string &option) {m_options.push_back(option);UpdateOptions();}
