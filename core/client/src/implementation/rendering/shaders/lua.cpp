...
pragma::LuaShaderWrapperGraphicsBase::LuaShaderWrapperGraphicsBase() : LuaShaderWrapperBase() {}
bool pragma::LuaShaderWrapperGraphicsBase::AttachVertexAttribute(const LuaVertexBinding &binding, const std::vector<LuaVertexAttribute> &attributes)
{
...
pragma::LuaShaderWrapperComputeBase::LuaShaderWrapperComputeBase() : LuaShaderWrapperBase() {}
...
pragma::LShaderGraphics::LShaderGraphics() : TLShaderBase<ShaderGraphics>(get_cengine()->GetRenderContext(), "", "", "") {}
void pragma::LShaderGraphics::InitializeDefaultRenderPass(std::shared_ptr<prosper::IRenderPass> &outRenderPass, uint32_t pipelineIdx) { ShaderGraphics::InitializeRenderPass(outRenderPass, pipelineIdx); }
...
pragma::LShaderCompute::LShaderCompute() : TLShaderBase<ShaderCompute>(get_cengine()->GetRenderContext(), "", "") {}
...
pragma::LShaderGui::LShaderGui() : TLShaderBase<Shader>(get_cengine()->GetRenderContext(), "", "", "") { SetPipelineCount(math::to_integral(gui::StencilPipeline::Count) * 2); }
...
bool pragma::LShaderGui::RecordBeginDraw(prosper::ShaderBindState &bindState, gui::DrawState &drawState, uint32_t width, uint32_t height, gui::StencilPipeline pipelineIdx, bool msaa, uint32_t testStencilLevel) const
{
	return Shader::RecordBeginDraw(bindState, drawState, width, height, pipelineIdx, msaa) && RecordSetStencilReference(bindState, testStencilLevel);
}
void pragma::LShaderGui::InitializeDefaultRenderPass(std::shared_ptr<prosper::IRenderPass> &outRenderPass, uint32_t pipelineIdx) { Shader::InitializeRenderPass(outRenderPass, pipelineIdx); }
...
pragma::LShaderGuiTextured::LShaderGuiTextured() : TLShaderBase<ShaderTextured>(get_cengine()->GetRenderContext(), "", "", "") {}
...
void pragma::LShaderGuiTextured::InitializeDefaultRenderPass(std::shared_ptr<prosper::IRenderPass> &outRenderPass, uint32_t pipelineIdx) { ShaderTextured::InitializeRenderPass(outRenderPass, pipelineIdx); }
...
pragma::LShaderParticle2D::LShaderParticle2D() : TLShaderBase<ShaderParticle2DBase>(get_cengine()->GetRenderContext(), "", "", "") {}
Vector3 pragma::LShaderParticle2D::DoCalcVertexPosition(const ecs::CParticleSystemComponent &ptc, uint32_t ptIdx, uint32_t localVertIdx, const Vector3 &camPos, const Vector3 &camUpWs, const Vector3 &camRightWs, float nearZ, float farZ) const
{
...
void pragma::LShaderParticle2D::InitializeDefaultRenderPass(std::shared_ptr<prosper::IRenderPass> &outRenderPass, uint32_t pipelineIdx) { ShaderParticle2DBase::InitializeRenderPass(outRenderPass, pipelineIdx); }
pragma::LuaShaderWrapperParticle2D::LuaShaderWrapperParticle2D() {}
Vector3 pragma::LuaShaderWrapperParticle2D::Lua_CalcVertexPosition(lua::State *l, ecs::CParticleSystemComponent &hPtC, uint32_t ptIdx, uint32_t localVertIdx, const Vector3 &camPos, const Vector3 &camUpWs, const Vector3 &camRightWs, float nearZ, float farZ)
{
...
void pragma::reset_lua_shaders()
{
	...
	for(auto &shader : shaderManager.GetShaders()) {
		auto *ls = dynamic_cast<LShaderBase *>(shader.get());
		if(ls)
			ls->SetLuaShader({}, nullptr);
	}
}
Vector3 pragma::LuaShaderWrapperParticle2D::DoCalcVertexPosition(const ecs::CParticleSystemComponent &ptc, uint32_t ptIdx, uint32_t localVertIdx, const Vector3 &camPos, const Vector3 &camUpWs, const Vector3 &camRightWs, float nearZ, float farZ) const
{
	auto ret = false;
	luabind::object r;
	auto &lPtc = ptc.GetLuaObject();
	if(const_cast<LuaShaderWrapperParticle2D *>(this)->CallLuaMember<luabind::object, luabind::object, uint32_t, uint32_t, const Vector3 &, const Vector3 &, const Vector3 &, float, float>("CalcVertexPosition", &r, lPtc, ptIdx, localVertIdx, camPos, camUpWs, camRightWs, nearZ, farZ)
...
pragma::LShaderImageProcessing::LShaderImageProcessing() : TLShaderBase<ShaderBaseImageProcessing>(get_cengine()->GetRenderContext(), "", "", "") { SetBaseShader<prosper::ShaderCopyImage>(); }
...
void pragma::LShaderImageProcessing::InitializeDefaultRenderPass(std::shared_ptr<prosper::IRenderPass> &outRenderPass, uint32_t pipelineIdx) { ShaderBaseImageProcessing::InitializeRenderPass(outRenderPass, pipelineIdx); }
...
pragma::LShaderPostProcessing::LShaderPostProcessing() : TLShaderBase<ShaderPPBase>(get_cengine()->GetRenderContext(), "", "", "") { SetBaseShader<prosper::ShaderCopyImage>(); }
...
pragma::LShaderGameWorldLightingPass::LShaderGameWorldLightingPass() : TLShaderBase<ShaderGameWorldLightingPass>(get_cengine()->GetRenderContext(), "", "", "")
{
	// SetBaseShader<ShaderTextured3DBase>();
}
pragma::LShaderGameWorldLightingPass::~LShaderGameWorldLightingPass() {}
void pragma::LShaderGameWorldLightingPass::RecordBindScene(rendering::ShaderProcessor &shaderProcessor, const CSceneComponent &scene, const CRasterizationRendererComponent &renderer, prosper::IDescriptorSet &dsScene, prosper::IDescriptorSet &dsRenderer,
  prosper::IDescriptorSet &dsRenderSettings, prosper::IDescriptorSet &dsShadows, const Vector4 &drawOrigin, SceneFlags &inOutSceneFlags) const
{
	...
	if(m_pushConstants->GetSize() > 0)
		shaderProcessor.GetCommandBuffer().RecordPushConstants(shaderProcessor.GetCurrentPipelineLayout(), prosper::ShaderStageFlags::VertexBit | prosper::ShaderStageFlags::FragmentBit, sizeof(PushConstants), m_pushConstants->GetSize(),
		  const_cast<LShaderGameWorldLightingPass *>(this)->m_pushConstants->GetData());

	m_bindUserData.Set<prosper::IShaderPipelineLayout>(string::string_switch::hash("pipelineLayout"), shaderProcessor.GetCurrentPipelineLayout());
...
std::shared_ptr<prosper::IDescriptorSetGroup> pragma::LShaderGameWorldLightingPass::InitializeMaterialDescriptorSet(material::CMaterial &mat)
{
	if(!m_wrapper)
		return nullptr;
	auto &shader = *dynamic_cast<Shader *>(this);
...
void pragma::LShaderGameWorldLightingPass::InitializeMaterialData(const material::CMaterial &mat, const rendering::shader_material::ShaderMaterial &shaderMat, rendering::ShaderInputData &inOutMatData)
{
...
void pragma::LShaderGameWorldLightingPass::BaseInitializeMaterialData(material::CMaterial &mat, const rendering::shader_material::ShaderMaterial &shaderMat, rendering::ShaderInputData &inOutMatData)
{
...
void pragma::LuaShaderWrapperTextured3D::Lua_InitializeMaterialData(material::CMaterial &mat, const rendering::shader_material::ShaderMaterial &shaderMat, rendering::ShaderInputData &inOutMatData)
{
...
int32_t pragma::LuaShaderWrapperTextured3D::Lua_OnDraw(geometry::ModelSubMesh &mesh) { return math::to_integral(util::EventReply::Unhandled); }
...
void pragma::LuaShaderWrapperTextured3D::InitializeMaterialBuffer(prosper::IDescriptorSetGroup &descSet, material::CMaterial &mat, const rendering::ShaderInputData &matData)
{
...
void pragma::LuaShaderWrapperTextured3D::InitializeMaterialData(material::CMaterial &mat, const rendering::shader_material::ShaderMaterial &shaderMat, rendering::ShaderInputData &inOutMatData)
{
	CallLuaMember<void, material::CMaterial *, const rendering::shader_material::ShaderMaterial *, rendering::ShaderInputData *>("InitializeMaterialData", const_cast<material::CMaterial *>(&mat), &shaderMat, &inOutMatData);
}
...
pragma::LShaderPbr::LShaderPbr() : TLShaderBase<ShaderPBR>(get_cengine()->GetRenderContext(), "", "", "")
{
	// SetBaseShader<ShaderTextured3DBase>();
}
...
int32_t pragma::LuaShaderWrapperPbr::Lua_OnDraw(geometry::ModelSubMesh &mesh) { return math::to_integral(util::EventReply::Unhandled); }
...
void pragma::LuaShaderManager::RegisterShader(std::string className, luabind::object &o)
{
	...
	auto *oShaderGraphics = luabind::object_cast_nothrow<LuaShaderWrapperGraphicsBase *>(r, static_cast<LuaShaderWrapperGraphicsBase *>(nullptr));
	auto *shaderWrapper = oShaderGraphics ? dynamic_cast<LuaShaderWrapperBase *>(oShaderGraphics) : nullptr;
	if(shaderWrapper == nullptr) {
		auto *oShaderCompute = luabind::object_cast_nothrow<LuaShaderWrapperComputeBase *>(r, static_cast<LuaShaderWrapperComputeBase *>(nullptr));
		shaderWrapper = oShaderCompute ? dynamic_cast<LuaShaderWrapperBase *>(oShaderCompute) : nullptr;
		if(shaderWrapper == nullptr) {
	...
	if(shader.expired()) {
		...
		if(shaderType == ShaderType::Graphics) {
			...
			auto &shaderCompute = *static_cast<LuaShaderWrapperCompute *>(shaderWrapper);
