...
pragma::ecs::CParticleSystemComponent *pragma::util::create_smoke_trail_particle(float distance, float speed, float minSpriteSize, float maxSpriteSize, const std::string &material)
{
	...
	auto strLifeTime = util::to_string(lifeTime);
	auto spread = (minSpriteSize / 45.f) * 0.1f;
	auto strSpread = util::to_string(spread);
	pt->AddInitializer("radius_random", std::unordered_map<std::string, std::string> {{"radius_min", util::to_string(minSpriteSize)}, {"radius_max", util::to_string(maxSpriteSize)}});
	...
	pt->AddInitializer("initial_velocity", std::unordered_map<std::string, std::string> {{"velocity", "0 " + util::to_string(speed) + " 0"}, {"spread_min", "-" + strSpread + "0 -" + strSpread}, {"spread_max", strSpread + " 0 " + strSpread}});
	...
	pt->AddOperator("color_fade", std::unordered_map<std::string, std::string> {{"color", "255 255 255 0"}, {"fade_start", util::to_string(math::max(lifeTime - 0.66f, 0.f))}, {"fade_end", strLifeTime}});
