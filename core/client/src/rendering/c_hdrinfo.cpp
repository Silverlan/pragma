...
const Vector3 &Scene::HDRInfo::Exposure::UpdateColor()
{
...
Scene::HDRInfo::HDRInfo()
	: //texture(nullptr),framebuffer(nullptr),renderPass(nullptr), // prosper TODO
	exposure(1.f),m_bMipmapInitialized(false),//textureBloom(nullptr), // prosper TODO
	//textureSwap(nullptr),framebufferSwap(nullptr),renderPassSwap(nullptr) // prosper TODO
	//,blurFramebuffer(nullptr), // prosper TODO
	m_exposure{},//descSetDepth(nullptr), // prosper TODO
	//descSetSwap(nullptr),cmdBufferBloom(nullptr), // prosper TODO
	//framebufferSwap2(nullptr), // prosper TODO
	/*descSetSwap2(nullptr),*/m_cbReloadCommandBuffer{} // prosper TODO
{
	//for(auto i=0;i<3;i++) // prosper TODO
	//	luminescence[i] = 0.f; // prosper TODO
...
bool Scene::HDRInfo::BeginRenderPass(std::shared_ptr<prosper::PrimaryCommandBuffer> &drawCmd,prosper::RenderPass *customRenderPass)
{
...
bool Scene::HDRInfo::EndRenderPass(std::shared_ptr<prosper::PrimaryCommandBuffer> &drawCmd)
{
	return prosper::util::record_end_render_pass(*(*drawCmd));
}

bool Scene::HDRInfo::ResolveRenderPass(std::shared_ptr<prosper::PrimaryCommandBuffer> &drawCmd)
{
	return EndRenderPass(drawCmd) && BeginRenderPass(drawCmd,rpIntermediate.get());
}

void Scene::HDRInfo::InitializeCommandBuffer()
{
	 // prosper TODO
	/*
...
bool Scene::HDRInfo::Initialize(Scene &scene,uint32_t width,uint32_t height,Anvil::SampleCountFlagBits sampleCount,bool bEnableSSAO)
{
...
bool Scene::HDRInfo::InitializeDescriptorSets()
{
...
bool Scene::HDRInfo::BlitMainDepthBufferToSamplableDepthBuffer(prosper::CommandBuffer &cmdBuffer,std::function<void(prosper::CommandBuffer&)> &fTransitionSampleImgToTransferDst)
{
...
bool Scene::HDRInfo::BlitStagingRenderTargetToMainRenderTarget(prosper::CommandBuffer &cmdBuffer)
{
...
void Scene::HDRInfo::SwapIOTextures() {m_curTex = (m_curTex == 1u) ? 0u : 1u;}
/*const Vulkan::Texture &Scene::HDRInfo::GetOutputTexture() const {return (m_curTex == 0u) ? textureSwap : texture->GetResolvedTexture();}
const Vulkan::Texture &Scene::HDRInfo::GetInputTexture() const {return (m_curTex == 0u) ? texture->GetResolvedTexture() : textureSwap;}
const Vulkan::DescriptorSet &Scene::HDRInfo::GetInputDescriptorSet() const {return (m_curTex == 0u) ? descSetSwap2 : descSetSwap;}

void Scene::HDRInfo::BeginRenderPass(Vulkan::CommandBufferObject *drawCmd)
{
	texture->Resolve();
...
void Scene::HDRInfo::EndRenderPass(Vulkan::CommandBufferObject *drawCmd)
{
	drawCmd->EndRenderPass();
	SwapIOTextures();
}*/ // prosper TODO

/*const Vulkan::Texture &Scene::HDRInfo::GetRenderTexture() const {return texture->GetTexture();}
const Vulkan::Texture &Scene::HDRInfo::GetTargetTexture() const {return texture->GetResolvedTexture();}

const Vulkan::Texture &Scene::HDRInfo::GetTargetBloomTexture() const {return textureBloom->GetResolvedTexture();}
const Vulkan::Texture &Scene::HDRInfo::GetTargetDepthTexture() const {return prepass.textureDepth->GetResolvedTexture();}*/ // prosper TODO

void Scene::HDRInfo::ResetIOTextureIndex() {m_curTex = 0u;}

void Scene::HDRInfo::UpdateExposure(prosper::Texture &srcTexture)
{
