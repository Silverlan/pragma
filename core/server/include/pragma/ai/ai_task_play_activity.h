...
namespace pragma
{
	namespace ai
	{
		class DLLSERVER TaskPlayActivity
		...
		public:
			...
			{
				Activity = umath::to_integral(TaskPlayAnimation::Parameter::Animation),
				FaceTarget = umath::to_integral(TaskPlayAnimation::Parameter::FaceTarget)
			};
		...
		class DLLSERVER TaskPlayLayeredActivity
			: public TaskPlayLayeredAnimation
		{
		protected:
			virtual int32_t SelectAnimation(const Schedule *sched,pragma::SAIComponent &ent,uint8_t paramId=0) override;
		public:
			enum class Parameter : uint32_t
			{
				Activity = umath::to_integral(TaskPlayLayeredAnimation::Parameter::Animation),
				AnimationSlot = umath::to_integral(TaskPlayLayeredAnimation::Parameter::AnimationSlot)
			};
