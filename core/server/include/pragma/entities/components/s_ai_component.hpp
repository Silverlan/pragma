...
{
public:
	NPCRelationship(const std::shared_ptr<void> &userData,int pPriority=0)
		: data(userData),priority(pPriority)
	{}
...
namespace pragma {class SCharacterComponent; class BaseActorComponent; class SPlayerComponent; enum class FPlayAnim : uint32_t;};

class SBaseEntity;
namespace pragma
{
	namespace ai {class Schedule; class BehaviorNode;};
	struct DLLSERVER CEMemoryData
	...
	struct DLLSERVER CEOnNPCStateChanged
		: public ComponentEvent
	{
	...
	struct DLLSERVER CEOnTargetAcquired
		: public ComponentEvent
	{
	...
	struct DLLSERVER CEOnControllerActionInput
		: public ComponentEvent
	{
	...
	struct DLLSERVER CEOnSuspiciousSoundHeared
		: public ComponentEvent
	{
	...
	struct DLLSERVER CEOnStartControl
		: public ComponentEvent
	{
	...
	struct DLLSERVER CEOnPathNodeChanged
		: public ComponentEvent
	{
	...
	struct DLLSERVER CEOnScheduleStateChanged
		: public ComponentEvent
	{
	...
	class DLLSERVER SAIComponent final
	...
	protected:
		...
		struct TargetInfo {
			TargetInfo(BaseEntity *_ent,float _dist)
				: ent(_ent),dist(_dist)
			{}
