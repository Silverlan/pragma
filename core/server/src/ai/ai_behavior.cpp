...
ai::BehaviorNode::BehaviorNode(Type type,SelectorType selectorType)
	: std::enable_shared_from_this<BehaviorNode>(),m_bActive(false),
	m_type(type)
{
	SetSelectorType(selectorType);
}
ai::BehaviorNode::BehaviorNode(SelectorType selectorType)
	: BehaviorNode(Type::Sequence,selectorType)
{}
...
ai::BehaviorNode::BehaviorNode::BehaviorNode(const BehaviorNode &other)
	: ParameterBase(other)
{
...
void ai::BehaviorNode::Print(const Schedule *sched,std::ostream &o) const
{
	o<<typeid(*this).name();
}
const std::vector<std::shared_ptr<ai::BehaviorNode>> &ai::BehaviorNode::GetNodes() const
{
	return m_childNodes;
}
