...
Faction::Faction(const std::string &name)
	: std::enable_shared_from_this<Faction>(),m_name(name),m_defaultDisp(DISPOSITION::NEUTRAL)
{}
...
void Faction::SetEnemyFaction(Faction &faction,bool revert,int priority)
{
	SetDisposition(faction,DISPOSITION::HATE,revert,priority);
}
void Faction::SetAlliedFaction(Faction &faction,bool revert,int priority)
{
	SetDisposition(faction,DISPOSITION::LIKE,revert,priority);
}
void Faction::SetNeutralFaction(Faction &faction,bool revert,int priority)
{
	SetDisposition(faction,DISPOSITION::NEUTRAL,revert,priority);
}
void Faction::SetFearsomeFaction(Faction &faction,bool revert,int priority)
{
	SetDisposition(faction,DISPOSITION::FEAR,revert,priority);
}
...
void Faction::SetDefaultDisposition(DISPOSITION disp)
{
	m_defaultDisp = disp;
}
DISPOSITION Faction::GetDefaultDisposition()
{
	return m_defaultDisp;
}
...
std::shared_ptr<Faction> FactionManager::RegisterFaction(const std::string &name)
{
	...
	auto it = std::find_if(m_factions.begin(),m_factions.end(),[&lname](const std::shared_ptr<Faction> &faction) {
		return (faction->GetName() == lname) ? true : false;
	});
