...
{
	auto *paramEvent = GetParameter(pragma::math::to_integral(Parameter::EventId));
	auto eventId = (paramEvent != nullptr) ? paramEvent->GetInt() : -1;
	o << "Event[" << eventId << "]";
	uint32_t argIdx = pragma::math::to_integral(Parameter::EventArgStart);
...
ai::BehaviorNode::Result ai::TaskEvent::Start(const Schedule *sched, pragma::BaseAIComponent &aiComponent)
{
	...
	auto *paramEventId = GetParameter(sched, pragma::math::to_integral(Parameter::EventId));
...
void ai::TaskEvent::SetEventId(AnimationEvent::Type eventId) { SetParameter(pragma::math::to_integral(Parameter::EventId), static_cast<int32_t>(eventId)); }
void ai::TaskEvent::SetEventArgument(uint32_t argIdx, const std::string &arg) { SetParameter(pragma::math::to_integral(Parameter::EventArgStart) + argIdx, arg); }
