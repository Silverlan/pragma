...
namespace {
	...
	auto UVN = register_variable<udm::Boolean>("sv_water_surface_simulation_shared", true, pragma::console::ConVarFlags::Archive, "If enabled, water surface simulation will be shared between client and server (Simulation is only performed once). This will only have an effect in single-player or on listen servers.");
