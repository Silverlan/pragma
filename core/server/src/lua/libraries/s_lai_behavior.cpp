...
namespace Lua
{
	namespace AIBehaviorNode
	{
		...
		static void GetParameterEntity(lua_State *l,ai::TaskWrapper &task,std::shared_ptr<pragma::ai::Schedule> &sched,uint8_t paramIdx,EntityHandle &def);
		...
		static void SetParameterEntity(lua_State *l,ai::TaskWrapper &task,uint8_t idx,EntityHandle &hEnt);
...
Lua::ai::TaskWrapper::TaskWrapper(pragma::ai::BehaviorNode &task)
	: TaskWrapper{task.shared_from_this()}
{}
...
AILuaBehaviorNode::AILuaBehaviorNode(Type type,pragma::ai::SelectorType selectorType)
	: pragma::ai::BehaviorNode(type,selectorType),LuaObjectBase()
{}
AILuaBehaviorNode::~AILuaBehaviorNode()
{}
void AILuaBehaviorNode::Print(const pragma::ai::Schedule *sched,std::ostream &o) const
{
	o<<ToString();
}
...
std::shared_ptr<pragma::ai::BehaviorNode> AILuaBehaviorNode::Copy() const
{
	return Lua::ai::server::create_lua_task(m_luaClass.interpreter(),m_luaClass,m_type,m_selector->GetType());
}
...
AILuaBehaviorNodeWrapper::AILuaBehaviorNodeWrapper(uint32_t nodeType)
	: AILuaBehaviorNodeWrapper(nodeType,umath::to_integral(pragma::ai::SelectorType::Sequential))
{}

AILuaBehaviorNodeWrapper::AILuaBehaviorNodeWrapper()
...
inline std::ostream &operator<<(std::ostream &s,const AILuaBehaviorNodeWrapper &task)
{
	return s<<typeid(AILuaBehaviorNode).name();
}
...
void Lua::AIBehaviorNode::register_class(lua_State *l,luabind::module_ &mod)
{
	...
	classDef.def("GetParameterEntity",static_cast<void(*)(lua_State*,ai::TaskWrapper&,std::shared_ptr<pragma::ai::Schedule>&,uint8_t,EntityHandle&)>(&GetParameterEntity));
	...
	classDef.def("SetParameterEntity",static_cast<void(*)(lua_State*,ai::TaskWrapper&,uint8_t,EntityHandle&)>(&SetParameterEntity));
...
void Lua::AIBehaviorNode::GetType(lua_State *l,ai::TaskWrapper &task)
{
	Lua::PushInt(l,umath::to_integral(task->GetType()));
}
void Lua::AIBehaviorNode::GetSelectorType(lua_State *l,ai::TaskWrapper &task)
{
	Lua::PushInt(l,umath::to_integral(task->GetSelectorType()));
}
void Lua::AIBehaviorNode::IsActive(lua_State *l,ai::TaskWrapper &task)
{
	Lua::PushBool(l,task->IsActive());
}
...
void Lua::AIBehaviorNode::SetScheduleParameter(lua_State*,ai::TaskWrapper &task,uint8_t taskParamId,uint8_t scheduleParamId)
{
	task->SetScheduleParameter(taskParamId,scheduleParamId);
}
void Lua::AIBehaviorNode::HasParameter(lua_State *l,ai::TaskWrapper &task,std::shared_ptr<pragma::ai::Schedule> &sched,uint8_t paramIdx)
{
	Lua::PushBool(l,(task->GetParameter(sched.get(),paramIdx) != nullptr) ? true : false);
}
void Lua::AIBehaviorNode::LinkParameter(lua_State *l,ai::TaskWrapper &task,uint8_t paramIdx,ai::TaskWrapper &taskOther)
{
	task->LinkParameter(paramIdx,*taskOther);
}
void Lua::AIBehaviorNode::LinkParameter(lua_State *l,ai::TaskWrapper &task,uint8_t paramIdx,ai::TaskWrapper &taskOther,uint8_t paramIdxOther)
{
	task->LinkParameter(paramIdx,*taskOther,paramIdxOther);
}
...
void Lua::AIBehaviorNode::GetParameterEntity(lua_State *l,ai::TaskWrapper &task,std::shared_ptr<pragma::ai::Schedule> &sched,uint8_t paramIdx,EntityHandle &def)
{
	//LUA_CHECK_ENTITY(l,def);
	auto *r = get_parameter<const BaseEntity*>(task,sched,paramIdx,pragma::ai::Schedule::Parameter::Type::Entity,&pragma::ai::BehaviorParameter::GetEntity,def.get());
	if(r != nullptr)
...
{
	EntityHandle def {};
	GetParameterEntity(l,task,sched,paramIdx,def);
}
...
void Lua::AIBehaviorNode::SetDebugName(lua_State *l,ai::TaskWrapper &task,const std::string &name)
{
	task->SetDebugName(name);
}
void Lua::AIBehaviorNode::GetDebugName(lua_State *l,ai::TaskWrapper &task)
{
	Lua::PushString(l,task->GetDebugInfo().debugName);
}
...
void Lua::AIBehaviorNode::SetParameterEntity(lua_State *l,ai::TaskWrapper &task,uint8_t idx,EntityHandle &hEnt)
{
	LUA_CHECK_ENTITY(l,hEnt);
	task->SetParameter(idx,hEnt.get());
}
...
void Lua::AIBehaviorNode::CreateTask(lua_State *l,ai::TaskWrapper &task,luabind::object o,pragma::ai::BehaviorNode::Type *taskType,pragma::ai::SelectorType *selectorType,int32_t aiTaskTypeParamOffset)
{
	create_task(l,*task,o,taskType,selectorType,aiTaskTypeParamOffset);
}
