...
pragma::networking::ClientRecipientFilter::ClientRecipientFilter(const std::function<bool(const IServerClient&)> &filter)
	: m_filter{filter}
{}
pragma::networking::ClientRecipientFilter::ClientRecipientFilter(const IServerClient &client,FilterType filterType)
{
	...
	case FilterType::Include:
		m_filter = [&client](const IServerClient &clOther) -> bool {
			return &clOther == &client;
		};
		break;
...
pragma::networking::ClientRecipientFilter::ClientRecipientFilter()
	: ClientRecipientFilter{[](const IServerClient&) -> bool {return true;}}
{}
...
{
	auto it = std::find_if(m_recipients.begin(),m_recipients.end(),[&client](const util::WeakHandle<const pragma::networking::IServerClient> &hClientOther) {
		return hClientOther.get() == &client;
	});
...
{
	return std::find_if(m_recipients.begin(),m_recipients.end(),[&client](const util::WeakHandle<const pragma::networking::IServerClient> &hClientOther) {
		return hClientOther.get() == &client;
	}) != m_recipients.end();
}
...
pragma::networking::TargetRecipientFilter::operator pragma::networking::ClientRecipientFilter() const
{
	...
	return ClientRecipientFilter{[recipients,filterType](const IServerClient &client) -> bool {
		auto result = std::find_if(recipients.begin(),recipients.end(),[&client](const util::WeakHandle<const pragma::networking::IServerClient> &hClientOther) {
			return &client == hClientOther.get();
		}) != recipients.end();
