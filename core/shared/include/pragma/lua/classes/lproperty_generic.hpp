...
enum class ArithmeticFloatPropertyType : uint32_t
{
	Float = 0u,
	Double,
	LongDouble
...
enum class ArithmeticIntPropertyType : uint32_t
{
	Int8 = 0u,
	UInt8,
	Int16,
...
template<typename TEnum>
	LGenericIntPropertyWrapper::LGenericIntPropertyWrapper(const util::PEnumProperty<TEnum> &prop)
		: m_property(prop)
{
...
template<class TProperty,typename T>
	class LSimplePropertyWrapper
		: public LBasePropertyWrapper
{
public:
	LSimplePropertyWrapper(const T &v)
		: LBasePropertyWrapper()
	{
		Initialize<TProperty,T>(v);
	}
...
public:
	...
	bool operator==(const TLNumberPropertyWrapper<TProperty,T> &propOther) const
	{
		return GetProperty() == *propOther;
	}
