...
namespace luabind
{
			...
			: native_converter_base<uint64_t>
	{
		...
		uint64_t to_cpp_deferred(lua_State *l,int index)
		{
			return static_cast<uint64_t>(lua_tonumber(l,index));
		}

		void to_lua_deferred(lua_State *l,uint64_t value)
		{
			...
			: native_converter_base<int64_t>
	{
		...
		int64_t to_cpp_deferred(lua_State *l,int index)
		{
			return static_cast<int64_t>(lua_tonumber(l,index));
		}

		void to_lua_deferred(lua_State *l,int64_t value)
		{
			lua_pushnumber(l,static_cast<lua_Number>(value));
		}
	};
