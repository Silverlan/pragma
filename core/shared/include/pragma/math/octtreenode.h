...
{
public:
	template<int> friend class OctTreeNode;
protected:
...
template<int nodeCount>
	OctTreeNode<nodeCount>::OctTreeNode(OctTreeNode<nodeCount> *parent)
		: BaseOctTreeNode(parent)
{
...
template<int nodeCount>
	unsigned char OctTreeNode<nodeCount>::GetChildCount() {return nodeCount;}
template<int nodeCount>
	OctTreeNode<nodeCount> *OctTreeNode<nodeCount>::GetParent() {return static_cast<OctTreeNode<nodeCount>*>(m_parent);}
template<int nodeCount>
	OctTreeNode<nodeCount> *OctTreeNode<nodeCount>::GetChild(unsigned char idx) {return m_children[idx];}
