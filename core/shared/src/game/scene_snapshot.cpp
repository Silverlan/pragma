...
std::shared_ptr<SceneSnapshot> SceneSnapshot::Create()
{
	return std::shared_ptr<SceneSnapshot>{new SceneSnapshot{}};
}
...
void SceneSnapshot::MergeMeshesByMaterial()
{
	...
	for(auto &pair : matMeshes)
	{
		...
		for(auto i=decltype(meshes.size()){1u};i<meshes.size();++i)
		{
			...
			auto itMeshOther = std::find_if(m_meshes.begin(),m_meshes.end(),[meshOther](const std::shared_ptr<Mesh> &mesh) {
				return mesh.get() == meshOther;
			});
