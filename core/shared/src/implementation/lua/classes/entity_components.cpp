...

{
	...
	def.add_static_constant("FATTACHMENT_MODE_UPDATE_EACH_FRAME", umath::to_integral(pragma::FAttachmentMode::UpdateEachFrame));


	def.add_static_constant("FATTACHMENT_MODE_PLAYER_VIEW", umath::to_integral(pragma::FAttachmentMode::PlayerView));


	def.add_static_constant("FATTACHMENT_MODE_PLAYER_VIEW_YAW", umath::to_integral(pragma::FAttachmentMode::PlayerViewYaw));


	def.add_static_constant("FATTACHMENT_MODE_SNAP_TO_ORIGIN", umath::to_integral(pragma::FAttachmentMode::SnapToOrigin));


	def.add_static_constant("FATTACHMENT_MODE_FORCE_TRANSLATION_IN_PLACE", umath::to_integral(pragma::FAttachmentMode::ForceTranslationInPlace));


	def.add_static_constant("FATTACHMENT_MODE_FORCE_ROTATION_IN_PLACE", umath::to_integral(pragma::FAttachmentMode::ForceRotationInPlace));


	def.add_static_constant("FATTACHMENT_MODE_FORCE_IN_PLACE", umath::to_integral(pragma::FAttachmentMode::ForceInPlace));
...
static std::optional<std::tuple<std::shared_ptr<ModelMesh>, std::shared_ptr<ModelSubMesh>, msys::Material *>> FindAndAssignSurfaceMesh(pragma::BaseSurfaceComponent &c, luabind::object oFilter)


{


	std::function<int32_t(ModelMesh &, ModelSubMesh &, msys::Material &, const std::string &)> filter = nullptr;


	if(oFilter) {


		filter = [&oFilter](ModelMesh &mesh, ModelSubMesh &subMesh, msys::Material &mat, const std::string &shader) -> int32_t {
	...
	return std::tuple<std::shared_ptr<ModelMesh>, std::shared_ptr<ModelSubMesh>, msys::Material *> {res->mesh->shared_from_this(), res->subMesh->shared_from_this(), res->material};
...

{
	...
	def.def("FindAndAssignSurfaceMesh", +[](pragma::BaseSurfaceComponent &c, luabind::object oFilter) -> std::optional<std::tuple<std::shared_ptr<ModelMesh>, std::shared_ptr<ModelSubMesh>, msys::Material *>> { return FindAndAssignSurfaceMesh(c, oFilter); });


	def.def("FindAndAssignSurfaceMesh", +[](pragma::BaseSurfaceComponent &c) -> std::optional<std::tuple<std::shared_ptr<ModelMesh>, std::shared_ptr<ModelSubMesh>, msys::Material *>> { return FindAndAssignSurfaceMesh(c, Lua::nil); });
