...
pragma::asset::IModelFormatHandler::IModelFormatHandler(util::IAssetManager &assetManager) : IAssetFormatHandler {assetManager} {}
...
///////////
std::unique_ptr<pragma::util::IAssetProcessor> pragma::asset::ModelLoader::CreateAssetProcessor(const std::string &identifier, const std::string &ext, std::unique_ptr<util::IAssetFormatHandler> &&formatHandler)
{
	auto processor = TAssetFormatLoader<ModelProcessor>::CreateAssetProcessor(identifier, ext, std::move(formatHandler));
...
///////////
pragma::asset::ModelLoadInfo::ModelLoadInfo(util::AssetLoadFlags flags) : AssetLoadInfo {flags} {}
pragma::asset::ModelProcessor::ModelProcessor(util::AssetFormatLoader &loader, std::unique_ptr<util::IAssetFormatHandler> &&handler) : FileAssetProcessor {loader, std::move(handler)} {}
...
pragma::asset::ModelManager::ModelManager(NetworkState &nw) : m_nw {nw}
{
	...
	auto addBlenderFormatHandler = [this](std::string ext) { return RegisterImportHandler(ext, [ext](IAssetManager &assetManager) -> std::unique_ptr<util::IImportAssetFormatHandler> { return std::make_unique<BlenderFormatHandler>(assetManager, ext); }); };
	...
	auto &assetManager = get_engine()->GetAssetManager();
	auto numImporters = assetManager.GetImporterCount(Type::Model);
	for(auto i = decltype(numImporters) {0u}; i < numImporters; ++i) {
		auto *importerInfo = assetManager.GetImporterInfo(Type::Model, i);
		if(!importerInfo)
			continue;
		for(auto &extInfo : importerInfo->fileExtensions) {
...
std::shared_ptr<pragma::asset::Model> pragma::asset::ModelManager::CreateModel(uint32_t numBones, const std::string &mdlName) { return Model::Create<Model>(&m_nw, numBones, mdlName); }
std::shared_ptr<pragma::geometry::ModelMesh> pragma::asset::ModelManager::CreateMesh() { return m_nw.GetGameState()->CreateModelMesh(); }
std::shared_ptr<pragma::geometry::ModelSubMesh> pragma::asset::ModelManager::CreateSubMesh() { return m_nw.GetGameState()->CreateModelSubMesh(); }
std::shared_ptr<pragma::asset::Model> pragma::asset::ModelManager::CreateModel(const std::string &name, bool bAddReference, bool addToCache)
{
	...
	if(addToCache) {
		auto asset = pragma::util::make_shared<util::Asset>();
...
pragma::util::AssetObject pragma::asset::ModelManager::InitializeAsset(const util::Asset &asset, const util::AssetLoadJob &job)
{
...
std::shared_ptr<pragma::asset::Model> pragma::asset::ModelManager::LoadModel(FWMD &wmd,const std::string &mdlName) const
{

}
std::shared_ptr<pragma::asset::Model> pragma::asset::ModelManager::LoadModel(const std::string &cacheName,const std::shared_ptr<ufile::IFile> &file,const std::string &ext)
{
...
std::shared_ptr<pragma::asset::Model> pragma::asset::ModelManager::LoadModel(const std::string &mdlName,bool bReload,bool *outIsNewModel)
{
	if(outIsNewModel)
		*outIsNewModel = false;
