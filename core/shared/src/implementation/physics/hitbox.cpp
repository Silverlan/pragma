...
void pragma::Model::AddHitbox(uint32_t boneId, HitGroup group, const Vector3 &min, const Vector3 &max) { AddHitbox(boneId, Hitbox(group, min, max)); }
void pragma::Model::AddHitbox(uint32_t boneId, const Hitbox &hitbox)
{
...
const std::unordered_map<uint32_t, Hitbox> &pragma::Model::GetHitboxes() const { return const_cast<pragma::Model *>(this)->GetHitboxes(); }
std::unordered_map<uint32_t, Hitbox> &pragma::Model::GetHitboxes() { return m_hitboxes; }
const Hitbox *pragma::Model::GetHitbox(uint32_t boneId) const
{
	auto it = m_hitboxes.find(boneId);
	if(it == m_hitboxes.end())
		return nullptr;
...
HitGroup pragma::Model::GetHitboxGroup(uint32_t boneId) const
{
...
std::vector<uint32_t> pragma::Model::GetHitboxBones(HitGroup group) const
{
	std::vector<uint32_t> bones;
	GetHitboxBones(group, bones);
	return bones;
}
void pragma::Model::GetHitboxBones(HitGroup group, std::vector<uint32_t> &boneIds) const
{
...
std::unordered_map<pragma::animation::BoneId, Hitbox> pragma::Model::CalcHitboxes() const
{
	struct BoneBounds {
