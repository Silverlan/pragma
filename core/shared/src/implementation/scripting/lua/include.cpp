...
static std::optional<std::string> format_error_message(lua_State *l, Lua::StatusCode statusCode, pragma::scripting::lua::ErrorType errType) {
    if (!Lua::IsString(l, -1))
        return {};
    ...
    auto formattedMsg = pragma::scripting::lua::format_error_message(l, errorMessage, statusCode, &item.fileName, errType);
    item.errorMessage = formattedMsg;
    return formattedMsg;
}
...
pragma::scripting::lua::IncludeResult include_file(lua_State *l, const util::Path &fileName, pragma::scripting::lua::IncludeFlags flags, Lua::IncludeCache *optIncludeCache) {
    ...
    auto statusCode = Lua::ExecuteFile(l, includeFileName, [](lua_State *l) -> int32_t {
        auto newErrMsg = format_error_message(l, Lua::StatusCode::ErrorRun, pragma::scripting::lua::ErrorType::RuntimeError);
        if (!newErrMsg)
            return 0;
        ...
        auto newErrMsg = format_error_message(l, statusCode, pragma::scripting::lua::ErrorType::LoadError);
        if (!newErrMsg)
            return;
    ...
    result.errorMessage = std::move(callInfo.errorMessage);
    return result;
}
