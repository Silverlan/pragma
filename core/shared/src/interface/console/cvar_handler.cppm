...
#pragma warning(disable : 4251)
	class DLLNETWORK CVarHandler {
	  ...
	  protected:
		...
		template<typename T>
		CallbackHandle RegisterConVarCallback(const std::string &scvar, const std::function<void(pragma::NetworkState *, const pragma::console::ConVar &, T, T)> &function);
		void FindSimilarConVars(const std::string &input, const std::map<std::string, std::shared_ptr<pragma::console::ConConf>> &cvars, std::vector<SimilarCmdInfo> &similarCmds) const;
	  ...
	  public:
		...
		pragma::console::ConVarFlags GetConVarFlags(std::string scmd);
		...
		bool GetConVarFlags(std::string scmd, pragma::console::ConVarFlags &outVal);

		virtual ConVarMap *GetConVarMap();
		std::map<std::string, std::shared_ptr<pragma::console::ConConf>> &GetConVars();
		const std::map<std::string, std::shared_ptr<pragma::console::ConConf>> &GetConVars() const;
		...
		std::shared_ptr<pragma::console::ConCommand> RegisterConCommand(const std::string &scmd, const std::function<void(pragma::NetworkState *, pragma::BasePlayerComponent *, std::vector<std::string> &, float)> &fc, pragma::console::ConVarFlags flags = pragma::console::ConVarFlags::None,
