	...
	namespace pragma::ecs {
		namespace events {
			struct DLLNETWORK CEOnBulletsFired : public ComponentEvent {
				CEOnBulletsFired(const BulletInfo &bulletInfo, const std::vector<pragma::physics::TraceResult> &hitTargets);
		...
		class DLLNETWORK BaseShooterComponent : public BaseEntityComponent {
		  public:
			...
			void GetBulletTraceData(const BulletInfo &bulletInfo, pragma::physics::TraceData &data) const;
			...
			virtual void FireBullets(const BulletInfo &bulletInfo, std::vector<pragma::physics::TraceResult> &outHitTargets, bool bMaster = true) = 0;
		  protected:
			...
			virtual pragma::physics::RayCastHitType OnBulletHit(const BulletInfo &bulletInfo, const pragma::physics::TraceData &data, pragma::physics::PhysObj &phys, physics::ICollisionObject &col);
