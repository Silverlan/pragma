...
export namespace luabind {
	template<typename TCpp, typename TLua>
	template<class U>
	TCpp cast_converter<TCpp, TLua>::to_cpp(lua_State *L, U u, int index)
	{
	...
	template<typename TCpp, typename TLua>
	template<class U>
	int cast_converter<TCpp, TLua>::match(lua_State *l, U u, int index)
	{
		return detail::default_converter_generator<TLua>::match(l, luabind::decorate_type_t<TLua> {}, index);
	}

	template<typename TCpp, typename TLua>
	void cast_converter<TCpp, TLua>::to_lua(lua_State *L, TCpp v)
	{
