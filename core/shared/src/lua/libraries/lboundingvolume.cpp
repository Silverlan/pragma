...
static void Transform_Box(const Mat3 &M,const Vector3 &min,const Vector3 &max,Vector3 &outNewMin,Vector3 &outNewMax)
{
	...
	Amin[0] = (float)min.x;  Amax[0] = (float)max.x;
	Amin[1] = (float)min.y;  Amax[1] = (float)max.y;
	Amin[2] = (float)min.z;  Amax[2] = (float)max.z;
	...
	outNewMin.x = Bmin[0];  outNewMax.x = Bmax[0];
	outNewMin.y = Bmin[1];  outNewMax.y = Bmax[1];
	outNewMin.z = Bmin[2];  outNewMax.z = Bmax[2];
} 
