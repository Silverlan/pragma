...
template<class InputItVert,class InputItIndex>
	...
	const Vector3 &waterPlaneRelObj,double waterPlaneDistRelObj,
	const Vector3 &waterVelocity,
	double gravity,
	InputItVert vertsBegin,
	InputItIndex indicesBegin,InputItIndex indicesEnd,
	double mass,
	double volBody,
	...
	static void calc_surface_plane(const PhysWaterSurfaceSimulator *surfaceSim,const Vector3 &pos,const Quat &rot,TIterator itVertBegin,TIterator itVertEnd,Vector3 &waterPlane,double &waterPlaneDist,Vector3 &waterPlaneRelObj,double &waterPlaneDistRelObj)
{
	...
	const std::array<Vector3,4> aabb2d = {
		Vector3(min.x,0.f,min.z), // TODO: Plane orientation
		Vector3(min.x,0.f,max.z),
		Vector3(max.x,0.f,min.z),
		Vector3(max.x,0.f,max.z)
	};
