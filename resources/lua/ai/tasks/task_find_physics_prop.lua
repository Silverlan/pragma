...
function ai.TaskFindPhysicsProp:__tostring() return "Find Physics Prop..." end
...
function ai.TaskFindPhysicsProp:FindPhysicsPropCandidate(schedule,npc)
	...
	if(trComponent == nil or charComponent == nil) then return end
	local origin = ent:GetCenter()
	local tr = charComponent:GetAimRayData()
	tr:SetSource(origin)
	local entClosest
	local distClosest = math.huge
	for ent in ents.iterator(bit.bor(ents.ITERATOR_FILTER_DEFAULT,ents.ITERATOR_FILTER_BIT_PHYSICAL),{ents.IteratorFilterSphere(origin,maxDist)}) do
...
function ai.TaskFindPhysicsProp:Start(schedule,npc)
	...
	if(entPhys == nil) then return ai.BehaviorTask.RESULT_FAILED end
