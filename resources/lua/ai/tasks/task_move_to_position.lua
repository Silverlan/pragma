...
function ai.TaskMoveToPosition:__tostring() return "Move to position..." end
...
function ai.TaskMoveToPosition:CheckTime()
	if(self.m_tEnd ~= nil and time.cur_time() > self.m_tEnd) then return false end
	return true
end
...
function ai.TaskMoveToPosition:Move(schedule,npc)
	...
	if(dist <= self.m_moveDistance) then return ai.BehaviorTask.RESULT_SUCCEEDED end
	if(self:CheckTime() == false) then return ai.BehaviorTask.RESULT_FAILED end
	npc:MoveTo(pos,self.m_aiMoveInfo)
	return ai.BehaviorTask.RESULT_PENDING
end

function ai.TaskMoveToPosition:Think(schedule,npc)
	if(self:HasParameter(schedule,self.PARAM_MOVE_TARGET) == false) then return ai.BehaviorTask.RESULT_FAILED end
	return self:Move(schedule,npc)
end

function ai.TaskMoveToPosition:GetMoveDistance() return self.m_moveDistance or 0.0 end
...
function ai.TaskMoveToPosition:Start(schedule,npc)
	...
	self.m_tEnd = self:HasParameter(schedule,self.PARAM_MAX_TIME) and (time.cur_time() +self:GetParameterFloat(schedule,self.PARAM_MAX_TIME)) or nil
