if(gui.skin_exists("default") == true) then return end
...
-- WIMainMenu
t.MENU_ITEM_COLOR = Color(145,145,145,255)
t.MENU_ITEM_SELECTED_COLOR = Color(255,255,255,255)
...
t.MENU_ITEM_SELECT_SOUND_FILE = "ui/ui_click_aftertap_mono"
t.MENU_ITEM_FONT_FILE = "vera/VeraBd.ttf"
...
t.TABLE_BACKGROUND_GRADIENT = function(t)
	t._TABLE_BACKGROUND_GRADIENT = t._TABLE_BACKGROUND_GRADIENT or create_gradient(t.TABLE_BACKGROUND_GRADIENT_START,t.TABLE_BACKGROUND_GRADIENT_END)
	return t._TABLE_BACKGROUND_GRADIENT
end
t.BUTTON_BACKGROUND_GRADIENT_HOVER = function(t)
	t._BUTTON_BACKGROUND_GRADIENT_HOVER = t._BUTTON_BACKGROUND_GRADIENT_HOVER or create_gradient(t.BUTTON_BACKGROUND_GRADIENT_HOVER_START,t.BUTTON_BACKGROUND_GRADIENT_HOVER_END)
	return t._BUTTON_BACKGROUND_GRADIENT_HOVER
end
t.BUTTON_BACKGROUND_GRADIENT_SELECTED = function(t)
	t._BUTTON_BACKGROUND_GRADIENT_SELECTED = t._BUTTON_BACKGROUND_GRADIENT_SELECTED or create_gradient(t.BUTTON_BACKGROUND_GRADIENT_SELECTED_START,t.BUTTON_BACKGROUND_GRADIENT_SELECTED_END)
...
skin["wibutton"] = {
	Initialize = function(GUI,pElement)
		...
		local fcCursorEntered = function()
			...
			if(pElement.m_bPressed == true) then return end
			if(pElement.m_pBackground == nil or not pElement.m_pBackground:IsValid()) then return end
...
skin["table_row"] = {
	Initialize = function(GUI,pElement)
		...
		local cbCursorEntered = pElement:AddCallback("OnCursorEntered",function()
			if(pElement:IsSelected()) then return end
	...
	children = {
		["witext"] = {
			Initialize = function(GUI,pElement)
				...
				if(pCell:IsValid() and pCell:GetClass() == "witablecell") then
					local fcSetSize = function()
						if(pElement:IsValid() == false) then return end
...
skin["wimainmenuelement"] = {
	Initialize = function(GUI,pElement)
		...
		if(pText ~= nil and pText:IsValid()) then
			...
			pElement.m_tSelected = 0
			pElement.m_bTransition = false
			local hThink = nil
			pElement:AddCallback("Select",function()
				...
				hThink = pElement:AddCallback("Think",function()
					local tCur = time.real_time()
					local tDelta = tCur -pElement.m_tSelected
					local scale = tDelta /GUI.MENU_ITEM_SHADOW_TRANSITION_TIME
					if(pText:IsValid()) then
						if(scale > 1.0) then
							scale = 1.0
							pElement.m_bTransition = false
							if(pElement.m_bSelected == false) then hThink:Remove() end
						end
						if(pElement.m_bSelected == false) then
							scale = 1.0 -(scale ^4)
						else
							scale = 1.0 -(1.0 -scale) ^4
						end
						local blurScale = scale *GUI.MENU_ITEM_SHADOW_BLUR
						local colStart = GUI.MENU_ITEM_SHADOW_COLOR
						local colEnd = GUI.MENU_ITEM_SELECTED_SHADOW_COLOR
