...
function gui.WIToolBar:AddTool(identifier,x,y,w,h)
	...
	el:AddCallback("OnSelected",function(el)
		...
		for _,toolOther in ipairs(self.m_tTools) do
			...
			if(pToolOther ~= el) then pToolOther:Deselect() end
		end
		self.m_selectedTool = el:GetIdentifier()
	end)
	return el
end
function gui.WIToolBar:GetSelectedTool() return self.m_selectedTool end
...
function gui.WIToolBarTool:SetIdentifier(identifier) self.m_identifier = identifier end
function gui.WIToolBarTool:GetIdentifier() return self.m_identifier end
...
function gui.WIToolBarTool:SetClipping(x,y,w,h)
	if(util.is_valid(self.m_pMain) == false) then return end
	self.m_pMain:SetClipping(x,y,w,h)
end
function gui.WIToolBarTool:IsSelected() return self.m_bSelected end
function gui.WIToolBarTool:ResetBackgroundColor()
	if(self:IsSelected() == true) then self:SetBackgroundColor(Color.Azure)
	else self:SetBackgroundColor(Color.Beige) end
end
...
function gui.WIToolBarTool:IsEnabled() return self.m_bEnabled end
...
function gui.WIToolBarTool:OnCursorEntered()
	if(util.is_valid(self.m_pBg) == false or self:IsEnabled() == false) then return end
